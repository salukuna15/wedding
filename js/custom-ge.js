...
if (wishesForm) {
  wishesForm.addEventListener('submit', function (event) {
    ...
    if (wishesName.length < 5) {
      displayErrorMessage(
        'სახელი უნდა იყოს მინიმუმ 5 სიმბოლო.',
        wishesForm,
      );
      return;
    }
    ...
    if (!wishesEmail.includes('@')) {
      displayErrorMessage('ელფოსტა უნდა იყოს ვალიდური.', wishesForm);
      return;
    }
...
if (attendingForm) {
  attendingForm.addEventListener('submit', function (event) {
    ...
    if (!attendingEmail.includes('@')) {
      displayErrorMessage('ელფოსტა უნდა იყოს ვალიდური', attendingForm);
      return;
    }

    // Validation on the phone
    if (attendingPhone.length < 9) {
      displayErrorMessage(
        'ტელეფონის ნიმერი უნდა იყოს მინიმუმ 9 სიმბოლო.',
        attendingForm,
      );
      return;
    }
    ...
    if (attendingAddress.length < 10) {
      displayErrorMessage(
        'მისამართი უნდა იყოს მინიმუმ 10 სიმბოლო.',
        attendingForm,
      );
      return;
    }
